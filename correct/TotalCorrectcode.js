...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function resetMapAndFetchStores() {
    // Reset map view to original position
    map.setView(originalPosition, originalZoom);

    // Fetch stores based on current filters
    ...
    if (category) {
      ...
      if (location) {
        // Use the fetchStoresByLocationAndStatus function here
        fetchStoresByLocationAndStatus(location, status);
      } else {
  ...
  document
    ...
    .addEventListener("change", function () {
      ...
      if (location) {
        fetchStoresByLocationAndStatus(location, status, category);
      } else {
        if (category) {
          fetchStoresByCategoryAndStatus(category, status);
        } else {
    ...
    distanceSlider.addEventListener("input", function () {
      ...
      if (category) {
        ...
        if (distance >= 20 && distance <= 30) {
          ...
          fetchNearbyStores(21.08591, 79.04498, distance, status);
        } else {
          fetchStoresByFilters("", status, distance);
        }
      }
    ...
    statusSwitch.addEventListener("change", function () {
      ...
      if (category) {
        ...
        if (defaultDistance >= 20 && defaultDistance <= 30) {
          ...
          fetchNearbyStores(21.08591, 79.04498, defaultDistance, status);
        } else {
          console.log("Fetching stores with:", {
            category: "",
            status,
  ...
  function fetchStoresByFilters(category, status, distance) {
    const latitude = 21.14979;
    const longitude = 79.08069;
  ...
  function populateStoreList(data) {
    ...
    const nagpurCoordinates = { lat: 21.14979, lng: 79.08069 };
  ...
  function populateCategories(data) {
    ...
    categorySelect.addEventListener("change", () => {
      ...
      const currentStatus = statusSwitch.checked ? "open" : "close";
  ...
  function fetchStoresByCategoryAndStatus(categoryName, openStatus) {
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        // Filter stores based on category, status, and today's working hours
        const filteredStores = data.filter((store) => {
          ...
          // If status is 'open', check if store is open today
          if (openStatus === "open") {
            return todayWorkingHours && todayWorkingHours.isOpen;
          }
          // If status is 'close', check if store is closed today
