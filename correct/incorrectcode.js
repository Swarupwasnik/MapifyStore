document.addEventListener("DOMContentLoaded", function () {
    ...
    //   const apiUrl = `http://localhost:5175/api/v1/stores/location-category-status?location=${encodeURIComponent(
    //     location
    //   )}&category=${encodeURIComponent(category)}&status=${status}`;
    ...
    // }
    function fetchStoresByLocationCategoryStatus(location, category, status) {
      const apiUrl = new URL("http://localhost:5175/api/v1/stores/location-category-status");
    ...
    document.getElementById("statusSwitch").addEventListener("change", function () {
      ...
      if (location) {
          // If location is entered, fetch stores by location, category, and status
          fetchStoresByLocationCategoryStatus(location, category, status);
      } else if (category) {
          // If category is selected, fetch stores by category and status
          fetchStoresByCategoryAndStatus(category, status);
      } else {
          // If no location or category, fetch stores by status
    ...
    document.getElementById("searchLocation").addEventListener("input", function () {
      ...
      const category = document.getElementById("category").value;
      const status = document.getElementById("statusSwitch").checked ? "open" : "closed";
  
      if (searchText) {
        fetchStoresByLocationCategoryStatus(searchText, category, status);
      } else {
        // If search is cleared, fetch stores by category and status
        if (category) {
          fetchStoresByCategoryAndStatus(category, status);
        } else {
          fetchStoresByStatus(status);
  ...
  document.getElementById("category").addEventListener("change", function () {
    ...
    if (location) {
        // If location is already entered, fetch stores by location, category, and status
        fetchStoresByLocationCategoryStatus(location, category, status);
    } else {
        // If no location, fetch stores by category and status
        fetchStoresByCategoryAndStatus(category, status);
    }
  });
